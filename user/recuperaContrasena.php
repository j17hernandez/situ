...
if (!empty($_POST)) {
        ...
        if( emailExiste($email))
    {
        ...
        $url = 'http://'.$_SERVER["SERVER_NAME"].'/proyectoSitu/user/cambiaContrasena.php?id='.$idUsuario.'&token='.$token;
        ...
        echo enviarEmail($email, $nombre ,$asunto ,$cuerpo);
         if (enviarEmail($email, $nombre ,$asunto,$cuerpo)) {
           ...
           if ($conectarBD->query($sqlCambioCon) == TRUE) {
            ...
            $errors[]= "<div class='alert alert-success'><h2>Hemos enviado un correo electronico a la direccion $email para que restablescas tu contraseña</h2></div>";
              } 
        }
    }else{
